<div align="center">
    <img src="./public/assets/SecEngOpsAWSProject Architecture.svg" alt="Logo" width="100%" height="100%">
...
### **Level 1: Initial Setup and Deployment**
...
pipeline {
    ...
    stages {
        ...
        stage('Checkout from Git') {
            steps {
                git branch: 'main', url: 'https://github.com/ujjwaljamuar/DevSecOps-Project'
            }
        }
...
pipeline{
    ...
    stages {
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       ...
                       sh "docker tag netflix ujjwaljamuar/netflix:latest "
                       sh "docker push ujjwaljamuar/netflix:latest "
                    }
                }
            }
        }
        stage("TRIVY"){
            steps{
                sh "trivy image ujjwaljamuar/netflix:latest > trivyimage.txt"
            }
        }
        stage('Deploy to container'){
            steps{
                sh 'docker run -d --name netflix -p 8081:80 ujjwaljamuar/netflix:latest'
            }
        }
    }
}
...
1. **Implement Notification Services:**
    ...
    - now set email notifications
    - create app in gmail
    - add everything, email and password,
    - smtp.gmail.com, port 465
    - test email

    fill extended email too
    fill default trigger - always and failure
...
pipeline{
    ...
    stages {
        ...
        stage('Checkout from Git'){
            steps{
                git branch: 'main', url: 'https://github.com/ujjwaljamuar/DevSecOps-Project.git'
            }
        }
        stage("Sonarqube Analysis "){
            steps{
        ...
        stage('Deploy to container'){
            steps{
                sh "docker stop \$(docker ps -a -q --filter ancestor=ujjwaljamuar/netflix:latest)"
                sh "docker rm \$(docker ps -a -q --filter ancestor=ujjwaljamuar/netflix:latest)"
                sh "docker run -d -p 8081:80 ujjwaljamuar/netflix:latest"
            }
        }
    }
    post{
